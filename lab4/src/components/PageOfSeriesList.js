...
import {seriesListUrl} from '../key';


const PageOfSeriesList = (props) => {
    ...
    let previousLink = "/series/page/" + previous;
    let nextLink = "/series/page/" + next;
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                await axios.get(seriesListUrl + '&limit=20&offset=' + previousOffset);
                // if (data.code === 500) {
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                const { data } =  await axios.get(seriesListUrl + '&limit=20&offset=' + nextOffset);
...
export default PageOfSeriesList;
