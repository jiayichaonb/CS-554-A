	...
	public routes(app): void {
		...
		app.route('/api/movies').get(async (req: Request, res: Response) => {
			try {
				...
				// if (take == 0) skip = 100;
				const movieList = await movieData.getAllmovies(req.query.skip, req.query.take);
		...
		app.route('/api/movies').post(async (req: Request, res: Response) => {
			...
			try {
				const newMovie = await movieData.addMovie(movieInfo.title, movieInfo.cast, movieInfo.info, movieInfo.plot, movieInfo.rating, movieInfo.comments);
				res.json(newMovie);
			} catch (e) {
		...
		app.route('/api/movies/:id').put(async (req: Request, res: Response) => {
			...
			try {
				const updatedMovie = await movieData.updateMovie(req.params.id, movieInfo.title, movieInfo.cast, movieInfo.info, movieInfo.plot, movieInfo.rating);
		...
		app.route('/api/movies/:id').patch(async (req: Request, res: Response) => {
			...
			try {
				const updatedMovie = await movieData.updateMovie(req.params.id, updatedObject.title, updatedObject.cast, updatedObject.info, updatedObject.plot, updatedObject.rating);
